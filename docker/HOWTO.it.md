...
3. Clicca la scheda **Elaborazione**:
   ...
   - **Dati meteo ausiliari** — scegli la fonte di ozono / vento / pressione:
...
- **Regolare quanta memoria riserva Docker:** **Resources** → cursore **Memory**.
  Docker Desktop riserva questa quantità di RAM dal computer mentre è in
  esecuzione, *sia che Polymer stia lavorando o no* — quindi se il computer va a
  corto di memoria, **abbassalo** (**4 GB** bastano per un'immagine alla volta).
  Rialzalo verso **8 GB** solo se l'elaborazione si ferma con un messaggio
  "out of memory". Il container di Polymer da solo usa circa **250 MB** da fermo.
  Su Windows puoi impostarlo anche in `C:\Users\<tu>\.wslconfig`:
  `[wsl2]` su una riga, `memory=4GB` sulla successiva; poi esegui `wsl --shutdown`.
...
| La build si ferma con **"load metadata for docker.io/mambaorg/micromamba …"** o un errore di rete / TLS | Docker non è riuscito a raggiungere Docker Hub. Verifica di essere online; se usi una **VPN o un proxy aziendale**, disconnettila (o fai autorizzare `hub.docker.com` / `registry-1.docker.io`); aspetta un minuto e rifai doppio clic sul launcher. Il launcher ora riprova il download da solo. |
| Il download dei dati ausiliari fallisce con **"Name or service not known"** / **"No address associated with hostname"** | Il container non riesce a risolvere gli indirizzi internet. **Disconnetti la VPN** e premi *Riprova*. Spesso basta riavviare Docker Desktop (tasto destro sulla balena → **Restart**). Se la tua rete richiede un proprio server DNS, apri `docker/docker-compose.yml`, cambia le due righe `dns:` (`8.8.8.8` / `1.1.1.1`) con quell'indirizzo e riavvia dal launcher. |
