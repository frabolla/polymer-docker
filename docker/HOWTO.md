...
| Build stops with **"load metadata for docker.io/mambaorg/micromamba …"** or a network / TLS error | Docker could not reach Docker Hub. Check you are online; if you use a **VPN or a company proxy**, disconnect it (or have IT allow `hub.docker.com` / `registry-1.docker.io`); wait a minute and double-click the launcher again. The launcher now retries the download automatically. |
| Auxiliary-data download fails with **"Name or service not known"** / **"No address associated with hostname"** | The container cannot resolve internet addresses. **Disconnect any VPN** and press *Retry*. Restarting Docker Desktop (right-click the whale → **Restart**) often fixes it. If your network requires its own DNS server, open `docker/docker-compose.yml`, change the two `dns:` lines (`8.8.8.8` / `1.1.1.1`) to that address, and restart with the launcher. |
